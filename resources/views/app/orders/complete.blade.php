...
<div class="row align-items-center">
    <div class="col">
        <h2 class="section-title mb-0">Despachar Pedido</h2>        
    </div>
...
<div class="row">
                ...
                <div class="col">
                    <div class="form-group mb-0">
                        <div class="form-row align-items-center">
                            <div class="col-2">
                                <label for="customer" class="mb-0 c_label">Cliente adjunto:</label>
                            </div>
                            <div class="col">
                                <input type="text" class="form-control c_control" readonly value="{{ $order->client->firstname . ' ' . $order->client->lastname }}">
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="col-8">
        <form action="{{ route('add-sale') }}" method="post">
            @csrf
            <input type="hidden" name="customer" id="customer" value="{{ $order->customer }}">
                ...
                <div class="card-box-body">                
                    <table id="products_table" class="table">
                        ...
                        <tbody>
                            ...
                            <tr data-product="{{ $detail->product->external_id }}" data-max="{{ $detail->quantity }}">
                                <td>{{ $detail->product->name }} <input type="hidden" name="id_product[]" value="{{ $detail->product->id_product }}"></td>
                                <td><input type="number" name="quantity[]" id="quantity_{{ $detail->product->external_id }}" class="form-control" value="{{ $detail->quantity }}"></td>
                                <td class="text-center">$ <span>{{ number_format((float)$detail->product->price, 2, '.', '') }}</span></td>
                                <td class="text-center">$ <span>{{ number_format((float) ($detail->product->price * $detail->quantity), 2, '.', '') }}</span> <input type="hidden" name="parcial[]" value="{{ $detail->product->price * $detail->quantity }}"></td>
                                <td><a href="#" class="delete"><i class="far fa-trash-alt"></i></a></td>
                            </tr>
                            @endforeach 
                        </tbody>
                    </table>
                    <p class="empty-list {{ (count($order->details) > 0) ? 'hidden':'' }}" id="empty">(No se han añadido productos)</p>
                </div>              
            </div>
            <div class="border-box mb-4">
                <div class="row align-items-center">
                    <div class="col-8">
                        <div class="form-group mb-0">
                            <div class="form-row align-items-center">
                                <div class="col-4">
                                    <label for="payment_method" class="mb-0 c_label">Metodo de Pago:</label>
                                </div>
                                <div class="col">
                                    <select name="payment_method" id="payment_method" class="form-control c_control">
            ...
            <div class="row">
                <div class="col text-right">
                    <button type="submit" class="btn btn-primary btn-lg">Generar Venta</button>
                </div>
            </div>
        </form>  
    </div>
    ...
    function changeValues(){
        ...
        $('#products_table tbody tr input').on('change', function(){
            ...
            if(val <= max){
                ...
                $(this).parent().parent().find('td:nth-child(4) input').val(values[index].price * val);
            }else{
                $(this).val(max);
                totalObj.html(parseFloat(values[index].price * max).toFixed(2)); 
                $(this).parent().parent().find('td:nth-child(4) input').val(values[index].price * max); 
                ...
                success: function(data, status){
                    ...
                    if(exists){
                        ...
                        var row = ` <tr data-product="${ product.external_id }" data-max="${ max }">
                                        ...
                                        <td class="text-center">$ <span>${ parseFloat(product.price).toFixed(2) }</span> <input type="hidden" name="parcial[]" value="${ product.price }"></td>
                                        <td><a href="#" class="delete"><i class="far fa-trash-alt"></i></a></td>
                                    </tr>`;
...
<script>
    var array = "{{ $array }}";
    var aux = array.replace(/&quot;/g,'"');
    values = JSON.parse(aux)
    setTotal();  
    changeValues();
</script>
@endsection